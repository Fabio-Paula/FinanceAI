    ...
    <Card>
      <CardContent className="p-5 space-y-3">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span
            className={trendUp ? 'text-[hsl(var(--success))]' : 'text-[hsl(var(--destructive))]'}
          >
            {trend}
          </span>
          <span>{sub}</span>
...
function generateInsights(summary: DashboardSummary, topCategories: CategoryBreakdown[]) {
  ...
  if (summary.pendingReview > 0) {
    items.push({
      id: 1,
      type: 'info',
      text: `${summary.pendingReview} transaç${summary.pendingReview === 1 ? 'ão aguarda' : 'ões aguardam'} revisão de categoria.`,
    })
  }
  if (summary.balance > 0) {
    items.push({
      id: 2,
      type: 'success',
      text: `Saldo positivo de ${fmt(summary.balance)} este mês.`,
    })
  } else if (summary.balance < 0) {
    items.push({
      id: 3,
      type: 'warning',
      text: `Saldo negativo de ${fmt(Math.abs(summary.balance))} este mês.`,
    })
  }
  if (topCategories.length > 0) {
    ...
    items.push({
      id: 4,
      type: 'info',
      text: `Maior categoria de gasto: ${top.category_name} (${fmt(top.total)}, ${top.percentage.toFixed(0)}% do total).`,
    })
  }
  if (items.length === 0) {
...
export function DashboardPage() {
  ...
  const { data: trendData } = useQuery({
    ...
    queryFn: () =>
      apiGet<{ data: Array<{ month: string; income: number; expense: number; balance: number }> }>(
        '/api/dashboard/monthly-trend?months=7'
      ),
  })
  ...
  const { data: byCategoryData } = useQuery({
    ...
    queryFn: () =>
      apiGet<{ data: CategoryBreakdown[] }>(
        `/api/dashboard/by-category?month=${monthKey}&type=expense`
      ),
  })
          ...
          <div className="flex items-center justify-between pt-5 pb-5">
            ...
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-sm inline-block bg-[hsl(var(--success))]" />{' '}
                Receita
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-sm inline-block bg-[hsl(var(--destructive))]" />{' '}
                Despesa
              </span>
            </div>
          </div>
              ...
              {insights.map(({ id, type, text }) => (
                <div
                  ...
                  className={cn(
                    ...
                    type === 'success' &&
                      'bg-[hsl(var(--success))]/5 border-[hsl(var(--success))]/15'
                  )}
                >
